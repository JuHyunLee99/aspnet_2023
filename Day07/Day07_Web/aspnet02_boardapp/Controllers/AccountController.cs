    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Profile(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                user.PasswordHash = _userManager.PasswordHasher.HashPassword(user, model.Password); // 비밀번호 변경은 어렵다ㅠㅠ
                var result = await _userManager.UpdateAsync(user);

                if (result.Succeeded) // 프로필변경 성공했으면
                {
                    // 로그인한 뒤 localhost:7139/Home/Index 로 가라
                    await _signInManager.SignInAsync(user, isPersistent: false);
            ...
            return View(model); // 프로필변경 실퍠하면 그화면 그대로 유지
        }
